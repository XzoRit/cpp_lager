...
void ImGui::ShowDemoWindow(bool *p_open)
{
    ...
    ImGui::PushItemWidth(ImGui::GetFontSize() *
                         -12); // Use fixed width for labels (by passing a negative value), the rest goes to widgets. We
    ...
    if (ImGui::CollapsingHeader("Configuration"))
    {
        ...
        if (ImGui::TreeNode("Configuration##2"))
        {
            ...
            ImGui::SameLine();
            HelpMarker("Instruct Dear ImGui to render a mouse cursor for you. Note that a mouse cursor rendered via "
        ...
        {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a "
                               "window or a block. Tree nodes can be automatically expanded.");
            HelpMarker(
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
...

        {
            ...
            HelpMarker("USER:\nHold SHIFT or use mouse to select text.\n"
                       ...
                       "if you need to wire InputText() to a dynamic string type. See misc/cpp/imgui_stdlib.h for an "
    ...
    if (ImGui::TreeNode("Text"))
    {
        ...
        if (ImGui::TreeNode("Word Wrapping"))
        {
            ...
            ImGui::Text("The lazy dog is a good dog. This paragraph is made to fit within %.0f pixels. Testing a 1 "
        ...
        if (ImGui::TreeNode("UTF-8 Text"))
        {
            ...
            ImGui::TextWrapped("CJK text will only appears if the font was loaded with the appropriate CJK character "
                               "ranges. Call io.Font->AddFontFromFileTTF() manually to load extra character ranges. "
    ...
    {
        ImGuiIO &io = ImGui::GetIO();
        ImGui::TextWrapped("Below we are displaying the font texture (which is the only texture we have access to in "
                           "this demo). Use the 'ImTextureID' type as storage to pass pointers or identifier to your "
                           "own texture data. Hover the texture for a zoomed view!");
    ...
    if (ImGui::TreeNode("Color/Picker Widgets"))
    {
        ...
        HelpMarker("With the ImGuiColorEditFlags_NoInputs flag you can hide all the slider/text inputs.\nWith the "
                   "ImGuiColorEditFlags_NoLabel flag you can pass a non-empty label which will only be used for the "
        ...
        ImGui::SameLine();
        HelpMarker("ColorEdit defaults to displaying RGB inputs if you don't specify a display mode, but the user can "
                   "change it with a right-click.\n\nColorPicker defaults to displaying RGB+HSV+Hex if you don't "
...
// Note that the SliderScalar function has a maximum usable range of half the natural type maximum, hence the /2 below.
        ...
        ImGui::SameLine();
        HelpMarker("As with every widgets in dear imgui, we never modify values unless there is a user "
...
static void ShowDemoWindowLayout()
{
    ...
    {
        HelpMarker("BeginGroup() basically locks the horizontal position for new line. EndGroup() bundles the whole "
                   "group so that you can use \"item\" functions such as IsItemHovered()/IsItemActive() or SameLine() "
                   "etc. on the whole group.");
        ImGui::BeginGroup();
        {
            ImGui::BeginGroup();
            ImGui::Button("AAA");
    ...
    {
        {
            ...
            ImGui::SameLine();
            HelpMarker("This is testing the vertical alignment that gets applied on text to keep it aligned with "
                       "widgets. Lines only composed of text or \"small\" widgets fit in less vertical spaces than "
                       "lines with normal widgets.");
            ImGui::Indent();
    ...
    if (ImGui::TreeNode("Scrolling"))
    {
        ...
        ImGui::SameLine();
        HelpMarker("We expose this for testing because scrolling sometimes had issues with window decoration such as "
                   "menu-bars.");
        ...
        HelpMarker("Use SetScrollHereX() or SetScrollFromPosX() to scroll to a given horizontal position.\n\nUsing the "
                   ...
                   "top/bottom/left/right-most item will add an additional WindowPadding to reflect on reaching the "
                   "edge of the list.\n\nBecause the clipping rectangle of most window hides half worth of "
                   "WindowPadding on the left/right, using SetScrollFromPosX(+1) will usually result in clipped text "
                   "whereas the equivalent SetScrollFromPosY(+1) wouldn't.");
        ...
        HelpMarker("Horizontal scrolling for a window has to be enabled explicitly via the "
...
static void ShowDemoWindowMisc()
{
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ImGui::TreeNode("Mouse cursors"))
        {
            ...
            ImGui::SameLine();
            HelpMarker("Your application can render a different mouse cursor based on what ImGui::GetMouseCursor() "
                       "returns. If software cursor rendering (io.MouseDrawCursor) is set ImGui will draw the right "
                       "cursor for you, otherwise your backend needs to handle it.");
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref)
{
    ...
    if (ImGui::Button("Revert Ref"))
        style = *ref;
    ImGui::SameLine();
    HelpMarker("Save/Revert in local non-persistent storage. Default Colors definition are not affected. Use \"Export "
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
    {
        ...
        if (ImGui::BeginTabItem("Fonts"))
        {
            ...
            HelpMarker("Those are old settings provided for convenience.\nHowever, the _correct_ way of scaling your "
                       "UI is currently to reload your font at the designed size, rebuild the font atlas, and call "
