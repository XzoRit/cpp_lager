    ...
    if (g.ActiveId != id && init_make_active)
    {
        ...
        if (is_multiline)
            g.ActiveIdUsingKeyInputMask |=
                ...
                ((ImU64)1 << ImGuiKey_PageDown); // FIXME-NAV: Page up/down actually not supported yet by widget, but
                                                 // claim them ahead.
        if (flags &
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_one =
            ImMax(1.0f, (float)(int)((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags, const float *ref_col)
{
    ...
    if (value_changed)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            B = col[2];
            ColorConvertRGBtoHSV(R, G, B, H, S, V);
            if (S == 0 && memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) ==
    ...
    if (!is_leaf)
    {
        if (pressed)
        {
            ...
            if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
                toggled |=
                    IsMouseHoveringRect(ImVec2(arrow_x1, interact_bb.Min.y), ImVec2(arrow_x2, interact_bb.Max.y)) &&
...
bool ImGui::BeginMenu(const char *label, bool enabled)
{
        ...
        ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            ...
            tb.y = ta.y +
                   ImMax((tb.y - extra) - ta.y, -100.0f); // triangle is maximum 200 high to limit the slope and the
                                                          // bias toward large sub-menus // FIXME: Multiply by fb_scale?
