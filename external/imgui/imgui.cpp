...
void ImGui::ItemSize(const ImVec2 &size, float text_baseline_y)
{
    ...
    window->DC.CursorPos.x =
        (float)(int)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);                  // Next line
    ...
    if (first_begin_of_the_frame)
    {
            ...
            window->AutoFitFramesY <= 0)
        {
            if (g.IO.DisplaySize.x > 0.0f &&
                g.IO.DisplaySize.y > 0.0f) // Ignore zero-sized display explicitly to avoid losing positions if a window
...
#undef NAV_MAP_KEY
    ...
    IM_ASSERT(!g.NavScoringRectScreen.IsInverted()); // Ensure if we have a finite, non-inverted bounding box here will
